...
try:
    # torchaudio <= 2.8 에서는 여전히 공개심볼 download_asset이 존재 (단, deprecate)
    from torchaudio.utils import download_asset  # no underscore!
except Exception:
    # torchaudio >= 2.9 등: download_asset 제거됨 -> torch.hub로 대체 다운로드
    from torch.hub import download_url_to_file

    def download_asset(rel_path: str, dst_dir: str = ".") -> str:
        """
        Mirror of torchaudio.utils.download.download_asset for tutorial assets.
        Returns the local file path.
        """
        base = "https://download.pytorch.org/torchaudio/tutorial-assets"
        rel = rel_path.lstrip("/")
        url = f"{base}/{rel}"
        dst = Path(dst_dir) / Path(rel).name
